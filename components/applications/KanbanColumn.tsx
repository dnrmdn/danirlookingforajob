...
export function KanbanColumn({ status, applications }: KanbanColumnProps) {
  ...
  return (
    <div className="w-[320px] flex-shrink-0 flex flex-col gap-4 snap-start h-full">
      ...
      <div 
        ref={setNodeRef}
        className={`
          flex-grow flex flex-col gap-4 rounded-2xl min-h-[150px] transition-colors duration-200 p-1
      ...
      >
        ...
        {applications.length === 0 && (
          <div className="h-full min-h-[100px] border-2 border-dashed border-white/5 rounded-2xl flex items-center justify-center text-gray-600 text-sm font-medium">
            Drop to move
          </div>
        )}
      </div>
